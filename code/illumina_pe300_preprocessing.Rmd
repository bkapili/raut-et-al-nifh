...
ls *.fastq | sed 's/_R1_001.fastq//' | sed 's/_R2_001.fastq//' | uniq > sample_names.txt
...
for f in `cat sample_names.txt`; do
  cutadapt \
    ...
    "$f"_R1_001.fastq "$f"_R2_001.fastq > tmp.txt
...
Move all cutadapt files to new directory.
```{bash move to new directory}
# Create new directory and move trimmed fastq files
mkdir ../cutadapt
mv *_CUTADAPT.fastq ../cutadapt && mv cutadapt_log.txt ../cutadapt && mv prinseq_log.txt ../cutadapt
```
